...
第二步是对人物进行细致描述，首先描述人物的肤色、体型，随后按身体部位逐步描述，在描述身体部位时在参考顺序的基础之上，灵活合并描述或者细化描述，在描述时直接输出描述内容，按照“描述内容”，不按照“部位：描述内容”的格式。
...
输出格式请返回json格式,原文件提供了多少场景就输出多少场景，不要重复输出也不要遗漏输出，每个场景在输出时保持相同缩进（括号内是内容提示词，不用出现在输出结果中）：
{
    "XX-XX(场景编号)":{
        "主体":"（全部人物相对位置和姿势）",
        "画面视角焦距":"(画面的视角),(画面焦距)",
        "细致描述":"(人物身体特征和服装),(身体部位描述),(身体部位描述1),(身体部位描述2),(身体部位描述3)",
    },
}
                """
    temperature = 1.1
    ...
    output_dir = os.path.join("两步版本", "过程文件", "nygs", "6th提示词", "1.中景")
