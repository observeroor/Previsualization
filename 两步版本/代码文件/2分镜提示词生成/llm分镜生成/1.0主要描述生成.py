...
第一步中必须注意要求：如果存在多个人物，则需要一起描绘，围绕女性角色展现出人物的相互位置，最后加入画面的视角、画面焦距描述。
第二步是对人物进行逐一细致描述（先描述女性角色，大概率只用描述1女、1男），首先描述人物的头发脸部、身体特征，随后按身体部位逐步描述，在描述身体部位时在参考顺序的基础之上，灵活合并描述或者细化描述，在描述时直接输出描述内容，按照“描述内容”，不按照“部位：描述内容”的格式。
[头发脸部参考内容：头发颜色、发型（注意同一角色在所有场景中要保持一致）、头发的特殊状态（如被射精）、头发上的装饰、脸部表情（要根据当前角色心理状态进行推测，主要表现出女性的柔弱、欲拒还拒、妩媚淫荡、背德感等让男性产生性欲、爱欲的表情）、眼睛表情、嘴巴表情（如：因做爱而微微张开）、脸部的特殊状态（如：因性高潮而微红）、脸部的装饰物（如：小巧的黑色耳钉）]
[身体特征参考内容：体型（如：高挑的韩国女团模特身材）、肤色（如：光洁白嫩的皮肤）、整体穿着（如：穿着一件轻薄的白色蕾丝绑带内衣，身体近乎全裸）]
[身体部位参考顺序：躯干、颈部、胸部、背部，屁股、阴部、胳膊、手、大腿、小腿]
第二步中必须注意要求1：只描述而且必须全部描描述会被镜头看到、在画面中出现的部位，如果某个身体部位或其细节会出现在画面中，但是在场景中未描述则需要根据想象进行补齐。
