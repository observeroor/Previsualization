...
词语绘画任务：
这是一项将文字可视化的特殊任务。
在我提供的文件中有很多的场景，你需要扮演场景的旁观者或者扮演场景中的男性角色，将你看到的画面用词语进行客观视觉化描述。
在我提供的文件中有很多的场景，你需要扮演场景的旁观者或者扮演场景中的男性角色，将你看到的画面用词语进行客观视觉化描述。
这些客观视觉化描述将交给画师，画师会把他们转化成一幅画。
画师会严格按照你的描述绘画，只有在你的词语描绘中提及到了的内容会在画面中出现，因此你必须尽可能的描述详细、用语直白简单（简单指的是使用准确直观的词汇，而不包含任何隐喻、比喻、心理活动）。
你必须尽可能的描述详细、用语，而不包含任何隐喻、比喻、心理活动。
不要用句子进行描述，而是用一组一组用逗号分隔的短语进行描述。
首先要描述出场景中人物的状况，包括但不限于身体姿势、动作、画面的视角、画面焦距等（eg：一个女孩，双腿盘坐，坐在桌子上，双手举起，乳头被男人的揉搓，从上方俯视，[35mm photograph]）
第一步中必须注意要求：只围绕女性和动作进行描述，不要出现背景的描述
第二步是对角色进行逐一细致描述（先描述女性，大概率只用描述1女、1男），首先描述橘色的头发脸部、身体特征，随后按身体部位逐步描述，在描述身体部位时在参考顺序的基础之上，灵活合并描述或者细化描述，在描述时直接输出描述内容，按照“描述内容”，不按照“部位：描述内容”的格式。
[头发脸部参考内容：头发颜色、发型（注意同一角色在所有场景中要保持一致）、特殊状态（如被射精在脸上）、头发上的装饰、脸部表情（要根据当前角色心理状态进行推测，主要表现出女性的柔弱、欲拒还拒、妩媚淫荡、背德感等让男性产生性欲、爱欲的表情，不要出现会影响人物美的描述词）、眼睛表情、嘴巴表情（如：因做爱而微微张开）、脸部的特殊状态（如：因性高潮而微红）、脸部的装饰物（如：小巧的黑色耳钉）]
[身体特征参考内容：体型（如韩国女团模特身材）、肤色（如：光洁白嫩的皮肤）、整体穿着（如：轻薄的白色蕾丝绑带内衣，近乎全裸）]
[身体部位参考顺序：躯干、颈部、胸部、背部，屁股、阴部、胳膊、手、大腿、小腿]
如果某个身体部位或其细节会被你看到，但是在场景中未描述出来，则需要根据想象进行补齐。
...
输出格式请返回json格式,原文件提供了多少场景就输出多少场景，不要重复输出也不要遗漏输出， "XX-XX(场景编号)"前缩进为四个空格，（括号内是内容提示词，不出现在输出结果中;除"X"外的、未在括号内的内容，都是绝对格式，在输出结果中作为绝对格式保留，不进行变动）：
{
    ...
    output_dir = os.path.join("两步版本", "过程文件", "nygs", "6th提示词", "1.1中景")
